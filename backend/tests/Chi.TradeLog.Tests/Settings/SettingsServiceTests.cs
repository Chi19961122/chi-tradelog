...
public class SettingsServiceTests
{
    ...
    public async Task CreatePlatformAsync_GeneratesPrefixedId()
    {
        ...
        var platform = await service.CreatePlatformAsync(new CreatePlatformInfo { Name = "  New Broker  " });
    ...
    public async Task CreateAccountAsync_ReturnsNull_WhenPlatformMissing()
    {
        ...
        var account = await service.CreateAccountAsync(new CreateAccountInfo { PlatformId = "nope", Name = "X" });
    ...
    public async Task AddSymbolAsync_NormalizesToUpper()
    {
        ...
        await service.AddSymbolAsync("  aapl ");
    ...
    private class FakeSettingsRepository : ISettingsRepository
    {
        ...
        public Task<IReadOnlyList<PlatformDataModel>> GetPlatformsAsync(CancellationToken cancellationToken = default)
            => Task.FromResult(Platforms);

        public Task<IReadOnlyList<AccountDataModel>> GetAccountsAsync(CancellationToken cancellationToken = default)
            => Task.FromResult(Accounts);

        public Task<IReadOnlyList<string>> GetSymbolsAsync(CancellationToken cancellationToken = default)
        ...
        public Task<decimal> GetInitialCapitalAsync(CancellationToken cancellationToken = default)
            => Task.FromResult(InitialCapital);

        public Task<int> UpdateInitialCapitalAsync(decimal value, CancellationToken cancellationToken = default)
            => Task.FromResult(1);

        public Task<bool> PlatformExistsAsync(string id, CancellationToken cancellationToken = default)
        ...
        public Task<int> DeletePlatformAsync(string id, CancellationToken cancellationToken = default)
            => Task.FromResult(1);

        public Task InsertAccountAsync(AccountDataModel account, CancellationToken cancellationToken = default)
            => Task.CompletedTask;

        public Task<int> DeleteAccountAsync(string id, CancellationToken cancellationToken = default)
            => Task.FromResult(1);

        public Task<bool> InsertSymbolAsync(string ticker, CancellationToken cancellationToken = default)
        {
            InsertedSymbol = ticker;
            return Task.FromResult(true);
        }

        public Task<int> DeleteSymbolAsync(string ticker, CancellationToken cancellationToken = default)
            => Task.FromResult(1);

        public Task<bool> InsertTagAsync(string name, CancellationToken cancellationToken = default)
            => Task.FromResult(true);

        public Task<int> DeleteTagAsync(string name, CancellationToken cancellationToken = default)
            => Task.FromResult(1);
    }
}
