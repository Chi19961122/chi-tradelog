...
**分支工作流（2026-07-12 起強制）**：

- **master 是保護主線，禁止直接 commit**——任何變更（含文件）一律先開分支：
  `feat/…`、`fix/…`、`docs/…`、`chore/…`（例：`fix/journal-blank-start`）。
- 合併一律走 GitHub PR：**CI 綠燈 + 使用者審核**後由使用者合併；AI 不主動 push、不合併 PR。
- 一個分支對應一個任務（計畫檔的一個階段或一個修正），做完就合、不養長壽分支。
- 採 GitHub Flow（feature branch → PR → master）。不採完整 Git Flow 的
  develop/release 層——單人專案無版本發布節奏，多一層 develop 只增加同步成本；
  未來若有正式版本發布需求再升級。

**Commit 紀律**：

- **每個通過驗證梯的階段／功能，立即 commit（到分支）**。不准累積多個階段不 commit（本專案曾一次堆積六個
