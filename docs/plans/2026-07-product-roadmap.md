...
> 狀態：**已完成（八項全數實作，2026-07-07）** ｜ 使用者已確認以下八項（兩批確認：功能 1/3/4/7 為第一批，
> 2/5/6/8 為第二批「行為改變」主題），排序依「相依性 + 投報率」。
> 各項完成紀錄見對應 commit：範本持久化+日記提醒（R1/R2 同 commit）、真實 R（Migration 0012）、
> 進階統計、行為分析（GET /api/journal/all）、紀律檢核（Migration 0013）、全資料匯出（GET /api/export）、週報。
